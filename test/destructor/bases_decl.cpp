...
// Test destructor subcontracting with contract declaration functions.
...
struct t {
    ...
    void destructor(boost::contract::decl c) const {
        boost::shared_ptr<n_cnt const> old_n =
                BOOST_CONTRACT_OLDOF(c, n_cnt::eval(n));
        boost::contract::var contract = boost::contract::destructor(c, this)
            .postcondition([old_n] { // Still.. shall not capture this.
