...
int CONTRACT_MEMBER_BODY(customer_manager, count) ( void ) const
    { return customers_.size(); }

bool CONTRACT_MEMBER_BODY(customer_manager, id_active) (
        basic_customer_details::identifier const& id) const
{
    return customers_.find(id) != customers_.end();
}

std::string const& CONTRACT_MEMBER_BODY(customer_manager, name_for) (
        basic_customer_details::identifier const& id) const
{
...
void CONTRACT_MEMBER_BODY(customer_manager, add) (
        basic_customer_details const& details )
    { customers_.insert(std::make_pair(details.id, customer(details))); }

void CONTRACT_MEMBER_BODY(customer_manager, set_name) (
