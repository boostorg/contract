 ...
 *  dbc::copyable<> because Boost's is_copyable<> only works for built-in
 *  types. For user defined types, Boost's is_copyable always returns false
 *  unless special compiler support is provided (and GCC does not provide
 *  support for this, probably only VC++ supports this at the moment...).
 */
...
public:
    ...
    explicit post(const post<D>& derived): now(derived.now) {
        /** @todo[LC] Gives inker error on MVSC (but OK for GCC) -- why?
