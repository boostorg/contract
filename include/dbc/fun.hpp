...
#   define DBC_tparam_arg(z, n, data) BOOST_PP_CAT(typename A, n) \
        ...
        // A0 = none_, A1 = none_, ... (if spec, no " = none_").
...
private:
    ...
    public:
        ...
        typedef typename dbc::remove_copyable_<C>::type class_type;
...
public:
    template<class C, class B = no_base<C> >
    class mem {
    public:
...
//                ());
        ...
        typedef result_type (class_type::* mem_fun_type)
                // ( arg0_type, arg1_type, ... );
        ...
        explicit mem(): base_contract_(), 
            ...
            if (!boost::is_same<
                    base_contract_type, no_base<class_type> >::value) {
    ...
    protected:
        ...
        inline result_type call(
            ...
            if (pre_invariant_check || post_invariant_check) {
                ...
                invariant_check = !(
                    ((const dbc::object<class_type>&)self)
...
public:
     ...
     * fully created yet so invariant may not hold yet.
     */
    template<class C> // Constructors cannot have base.
    class constr: public mem<C, no_base<C> > {
    public:
        /** @todo[LC] Statically assert:
         * (1)  C is not dbc::copyable<>.
         */
        ...
        inline result_type operator()(
                // Only add ref here (it's const only if it was already const).
                typename boost::add_reference<class_type>::type self
                , typename mem<C, no_base<C> >::mem_fun_type body
                // , arg0_type arg0, arg1_type arg1, ...
                BOOST_PP_ENUM_TRAILING(DBC_arg_num, DBC_arg, ~)
                , const std::string& the_fun_name = ""
                ) {
...
public:
    ...
    template<class C> 
    class destr: public mem<C, no_base<C> > {
    public:
        /** @todo[LC] Statically assert:
         * (1)  C is not dbc::copyable<>.
         */
        ...
        // Make sure no check throws.
        
#if DBC_CHECK_INVARIANT
        inline void check_invariant(
                DBC_ADD_CONST_REF_(typename, class_type) self) throw() {
            try { mem<C, no_base<C> >::check_invariant(self); }
            catch (...) { broken_destructor_invariant(); } // Never throw.
        }
#endif // check
...
#   define DBC_post_var_decl
#   define DBC_post_var_name
#   define DBC_post_arg_const_ref
#   define DBC_arg_const_ref
#   define DBC_arg
#   define DBC_arg_type
#   define DBC_arg_name
#   define DBC_is_not_spec
#   define DBC_is_spec
#   define DBC_is_not_void
#   define DBC_is_void
#   define DBC_arg_num
#   define DBC_rest_arg_num
