...
#define DBC_CONTRACT_OP_(s, state, arg_name) \
    BOOST_PP_IF(BOOST_PP_IS_EMPTY(arg_name), state, \
        BOOST_PP_CAT(state, BOOST_PP_CAT(arg_name, _)))

#define DBC_CONTRACT(fun_name, arg_names) \
    BOOST_PP_CAT(dbc_contract_, \
        BOOST_PP_CAT(fun_name, BOOST_PP_CAT(_, \
        BOOST_PP_IF(BOOST_PP_SEQ_SIZE(arg_names), \
            BOOST_PP_SEQ_FOLD_LEFT, BOOST_PP_TUPLE_EAT(3) \
        )(DBC_CONTRACT_OP_, , arg_names) \
    )))
