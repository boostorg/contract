...
#define DBC_PP_FUN_PARSE_VALIDATE_(valid_traits_size, sign_traits_err) /**/ \
    ...
    /* one and one token only (BOOST_PP_EMPTY) should be left */ \
    , BOOST_PP_IF(BOOST_PP_NOT_EQUAL(BOOST_PP_SEQ_SIZE(BOOST_PP_TUPLE_ELEM( \
            3, 0, sign_traits_err)), 1), \
    ...
    , BOOST_PP_IF(BOOST_PP_NOT_EQUAL(valid_traits_size, BOOST_PP_SEQ_SIZE( \
        ...
        DBC_PP_FUN_PARSE_RETURN_ERR_(sign_traits_err, \
            ...
            DBC_INTERNAL_ERROR_wrong_number_of_traits_after_parsing_function_signature_ /* BOOST_PP_STRINGIZE(valid_traits_size) when BOOST_PP_STRINGIZE(BOOST_PP_SEQ_SIZE(BOOST_PP_TUPLE_ELEM(3, 1, sign_traits_err))) found */) \
