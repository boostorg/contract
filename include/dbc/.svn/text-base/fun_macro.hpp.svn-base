...
#ifndef DBC_FUN_MACRO_HPP_
#define DBC_FUN_MACRO_HPP_
...
#define DBC_MEM_FUN_CODE_(err, f, inner_contract_type, \
    ...
    /* contract */ \
protected: \
    friend class DBC_FUN_GET_MEM_CONTRACT_TYPE_(f, inner_contract_type); \
    ...
    class DBC_FUN_GET_CONTRACT_NAME_(f): \
            public DBC_FUN_GET_MEM_CONTRACT_TYPE_(f, inner_contract_type) { \
        BOOST_PP_EXPR_IF(DBC_CHECK_REQUIRE, \
                void require(DBC_FUN_GET_MEM_REQUIRE_ARGS_(f)) require_code) \
        BOOST_PP_EXPR_IF(DBC_CHECK_ENSURE, \
                void ensure(DBC_FUN_GET_MEM_ENSURE_ARGS_(f)) ensure_code) \
    }; \
    /* declare fun body */ \
...
#define DBC_MEM_FUN_ERROR_(err, f, inner_contract_type, \
        require_code, ensure_code, body_code) \
...
#define DBC_MEM_FUN_FROM_TRAITS_(sign_traits_err, inner_contract_type, \
        require_code, ensure_code, body_code) \
    ...
    (BOOST_PP_TUPLE_ELEM(3, 2, sign_traits_err), \
            BOOST_PP_TUPLE_ELEM(3, 1, sign_traits_err), inner_contract_type, \
            require_code, ensure_code, body_code) \

#endif // DBC

#endif // DBC_FUN_MACRO_HPP_
