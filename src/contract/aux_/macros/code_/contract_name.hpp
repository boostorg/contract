...
#define CONTRACT_AUX_CODE_CONTRACT_NAME_SEQ_(kind, sign) \
    ...
    BOOST_PP_EXPR_IF(CONTRACT_AUX_PP_SIGN_IS_CONST(sign), \
        ( const ) \
    ) /* cannot just concat empty seq () if non const */ \
    BOOST_PP_EXPR_IF(CONTRACT_AUX_PP_SIGN_IS_VOLATILE(sign), \
        ( volatile ) \
    ) /* cannot just concat empty seq () if non volatile */ \

#define CONTRACT_AUX_CODE_CONTRACT_NAME_PREFIXED(kind, sign, \
        prefix_seq) \
    CONTRACT_AUX_CODE_CONTRACT_NAME_FROM_SEQ_( \
            prefix_seq /* seq concat */ \
            CONTRACT_AUX_CODE_CONTRACT_NAME_SEQ_(kind, sign) \
    )

// Cannot just call macro above with prefix () because some compiler
// do not support empty seq elements.
