...
#ifndef CONTRACT_AUX_PP_SIGN_ARGS_HPP_
...
#define CONTRACT_AUX_PP_SIGN_ARGS(sign) \
    BOOST_PP_SEQ_ELEM(CONTRACT_AUX_PP_SIGN_SEQ_ARGS_INDEX, sign)

#define CONTRACT_AUX_PP_SIGN_HAS_ARGS(f) \
    BOOST_PP_NOT(BOOST_PP_IS_EMPTY(BOOST_PP_TUPLE_ELEM(3, 2, \
            BOOST_PP_SEQ_HEAD(CONTRACT_AUX_PP_SIGN_ARGS(f))) \
            (/* exapnd emtpy */) ))

// Expand in arg that can be inspected using macros in "arg.hpp".
#define CONTRACT_AUX_PP_SIGN_ARG(n, sign) \
    BOOST_PP_SEQ_ELEM(n, CONTRACT_AUX_PP_SIGN_ARGS(sign))
