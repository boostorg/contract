 ...
 * class public API).
 * 
 * This macro is used for convenience so the programmer does not have to 
 * manually write the private inheritance from @c dbc::object with the
 * <c>\#ifdef DBC</c> guard. For example (from @ref A_Complete_Example):
 * @code
 *  #include <dbc.hpp>
 *  
 ...
 * This macro is used for convenience so the programmer does not have to
 * manually write the private inheritance from @c dbc::object with the
 * <c>\#ifdef DBC</c> guard. For example:
 * @code
 *  #include <dbc.hpp>
 *  
 *  class B { ... };
 ...
 * This base object is used by the library implementation to store internal
 * state, etc.
 */
template<class C>
class object {};
