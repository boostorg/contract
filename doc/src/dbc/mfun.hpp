 ...
 *  <em>[</em>(template)<em>]</em> (class_type)( <em>{</em>(void) <em>||</em>
 ...
 * "Destructor Call Semantics" checking the class invariant (destructors have
 * no preconditions and postconditions). The contract specified by this macro
 * is automatically documented.
 * 
 * @remark The C++ Standard Template Library (STL) exception safety rules 
 *  require destructors to never throw exceptions. To comply with this STL 
 *  requirement, if the class invariant is violated while the destructor 
 *  contract is checked, no exception will be thrown and
 *  @c dbc::broken_destructor_invariant() will be invoked instead (this 
 *  function can be redefined by the user, it terminates
 *  by default). Furthermore, if the destructor is invoked during stack
 *  unwinding due to an unhandled exception then an invariant violation while
 *  checking the destructor contract will simply be ignored (no exception will
 *  be thrown, @c dbc::broken_destructor_invariant() will @e not be invoked,
 *  and only a warning message will be logged).
 * 
 * For example (from @ref A_Complete_Example):
 * @code
 *  #include <dbc.hpp>
 ...
 *  <em>{</em>(public) <em>||</em> (protected) <em>||</em> (private)<em>}</em>
 *  <em>[</em>(virtual)<em>]</em>
 *  <em>[</em>(template)<em>]</em> (class_type)( (void) )
 *  </c> @n @n
 *  Comparing this @a signature syntax with the one of @c DBC_MEM_FUN() note:
 ...
 * @see Different macros @c DBC_CONSTRUCTOR() and @c DBC_MEM_FUN() need to be
 *  used for constructor and member function contracts because of their
 *  different DBC call semantics.
 * @see @c DBC_INVARIANT() to specify class invariants.
 */
 ...
 * The contract specified by this macro is automatically documented (see
 ...
 *  (function_name)( <em>{</em>(void) <em>||</em>
 *  <em>{[</em>DBC_COPYABLE<em>]</em>(argument_type)(argument_name)<em>}*</em>
 *  <em>}</em>
 *  ) <em>[</em>(const)<em>]</em>
 *  </c> @n @n
 ...
 *      <c>@e expression</c>. For example, <c><em>{</em>(public) <em>||</em> 
 *      (protected) <em>||</em> (private)<em>}</em></c>
 ...
 *  @arg @c void must be specified for the argument list if the function has no
 *      no arguments -- simply <code>()</code> cannot be used instead because
 *      <code>()</code> is NOT a valid preprocessor sequence token in standard
 *      C++.
 * 
 * <small>
 * @note This library supports omitting <code>(void)</code> for empty function 
 *      argument lists but it will only compile successfully on C99. The C99
 *      preprocessor supports <code>()</code> (see @ref Boost
 *      "Boost.Preprocessor" documentation) plus the use of @c void to 
 *      specify empty function argument list is deprecated in C99. Therefore,
 *      if your code only has to compile on a C99 compiler, it is recommended
 *      to omit <code>(void)</code> for empty function argument lists and use
 *      the more intuitive <code>()</code> syntax.
 *
 ...
 *      Eventual argument default parameter are NOT indicated in the contract
 *      @a signature (they are only indicated in the signature of the member
 *      function preceding this macro call).
 * 
 ...
 *  @remark Well written preconditions must not use private or protected
 *      members to check the contract (otherwise, the caller will not be able
 *      to verify the preconditions are satisfied before calling the function
 *      as they depend on some private of protected member that is unaccessible
 *      to the caller). Eiffel enforces this rule strictly generating a
 *      compile-time error (see [@ref Mey97]).
 ...
 *  assertions (otherwise it is likely that the contract will have more bugs
 *  than the code it is checking).
 *  @remark Postconditions should use public members as much as possible so the
 *      contracts will be implementation independent. However, postconditions
 ...
 * @see Different macros @c DBC_CONSTRUCTOR() and @c DBC_DESTRUCTOR() need to
 *  be used for constructor and destructor contracts because of their different
 *  DBC call semantics.
 * @see @c DBC_INVARIANT() to specify class invariants.
 */
