 ...
 *
 * <small>
 * @note Name abbreviations @c fun and @c mem follows the convention introduced
 *  by C++ STL to name member function binders @c std::mem_fun for member
 *  functions. Similarly, the abbreviations @c constr and @c destr were chosen
 *  for constructor and destructor respectively.
 *
 * </small>
 */
...
public:
    ...
    */    
     ...
     *  @remark In general, to allow subcontracting it is necessary to make
     *      @c dbc::fun::mem and @c dbc::post friends for the contracted class
     *      (e.g., when subcontracting from private base class). This is
     *      automatically done by the macro-based API (and causes the
     *      annoying multiple friend compiler warning) but it must be done
     *      manually when using the code-based API.
     * @see @ref Code_Based_API for an example.
     */
    ...
    public:
        ...
        typedef R (uncopyable_class_type::* mem_fun_type)(
                uncopyable_arg0_type, uncopyable_arg1_type, ...);
         ...
         */
        inline virtual R call(
                uncopyable_class_type& self, mem_fun_type body,
    ...
    protected:
         ...
         *
         * <small>
         * @note Defining this function pure virtual instead of virtual
         *  <c>{}</c> allows to check at compile-time that the use is
         *  overriding the proper virtual function to specify the contract
         *  (otherwise an error in specifying the function signature would
         *  result in declaring a different virtual function and silently not
         *  checking the user defined contracts).
         *
         * </small>
         */
    ...
    public:
         ...
         * @param[in] name The function name (optional, used for log messages).
         * @remark This call return type is @c void because constructors return
         *  no value.
         */
    ...
    protected:
         ...
         * @param[in] arg0, arg1, ... Constant references to the argument
         *  values (present only if @a A0, @a A1, ... have been specified).
         * @remark There is no @c self argument as there is no object before
         ...
         *  specified @c dbc::copyable (and similarly for the other argument 
         *  numbers).
         * @remark There is no @a result argument because constructors return
         *  no value.
         */
        inline virtual void ensure(const post<copyable_class_type>& self,
                const post<copyable_arg0_type>& arg0,
    ...
    public:
         ...
         * @param[in] body A pointer to function implementing the body.
         * @param[in] name The function name (optional, used for log messages).
         * @remark There is no function argument type because destructors take
         *  no argument.
         * @remark This call returns @c void because destructors return no
         *  value.
         */
        inline void call(uncopyable_class_type& self, mem_fun_type body,
                const std::string& name = "");
