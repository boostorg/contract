...
- @c __UncopyableClassType__ is <c>__RemoveCopyable__< __RemoveCV__< ClassType > ></c>.
- If @c ClassType is cv-qualified then @c __MaybeCV__ is the cv-qualifier (@c const, @c volatile, or <c>const volatile</c>), otherwise it is nothing.
- If @c S is tagged @c contract::copyable then <c> __IfCopyable__< S, T ></c> is the @c T, otherwise it is the @c contract::noold.
- If @c F has a non-void @c ResultType then <c>__IfNonVoid__< C ></c> is @c C, otherwise it is nothing.
- @c __MaybeCvClassPointer__ is a pointer to @c __UncopyableClassType__ with the same cv-qualifier of @c ClassType (<c>*</c>, <c>const*</c>, <c>volatile*</c>, or <c>const volatile*</c>).
...
public:
    ...
    */
    virtual ResultType call(__MaybeCVClassPointer__ object,
